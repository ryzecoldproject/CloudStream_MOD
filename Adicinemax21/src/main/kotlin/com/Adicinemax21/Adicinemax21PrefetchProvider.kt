...
/**
 * Runtime layer that keeps the proven MovieBox/VidSrc wrapper untouched and
 * adds a prefetched Idlix race beside it.
 */
class Adicinemax21PrefetchProvider : Adicinemax21() {
    ...
    private val playbackDelegate = Adicinemax21PlaybackFixedProvider()
    ...
    private fun forwarder(
        ...
        if (!duplicateIdlix) {
            ...
            callback(link)
            Log.i("Adicinemax21", "[IDLIX-PREFETCH] callback|FAMILY=$key|LINKS=$count|FAMILIES=${families.size}")
    ...
    override suspend fun loadLinks(
                ...
                supervisorScope {
                    ...
                    val prefetchedIdlix = launch {
                        try {
                            ...
                            Log.e("Adicinemax21", "[IDLIX-PREFETCH] ${error.javaClass.simpleName}: ${error.message}")
                        }
                    }
        ...
        Log.i("Adicinemax21", "[IDLIX-PREFETCH] release|FAMILIES=${families.joinToString(",")}|LINKS=${emitted.get()}")
        emitted.get() > 0
    }
